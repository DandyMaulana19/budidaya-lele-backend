...
import { getFeederData } from "../controllers/mqtt.controller.js";
...
const getTempStatus = (temp) => {
  ...
  if ((temp >= 22.0 && temp < 25.0) || (temp > 32.0 && temp <= 34.0))
    return "warning";
  return "danger";
};
...
const notif = new AsyncTask("iot-notif-task", async () => {
  ...
  datas.forEach(async (item) => {
    ...
    const prevState = lastNotifState.get(nodeId) || {
      ph: "normal",
      temp: "normal",
    };
    ...
    if (phStatus !== "normal" && phStatus !== prevState.ph) {
      const title =
        phStatus === "warning" ? "⚠️ Peringatan pH" : "🚨 Bahaya pH";
    ...
    if (tempStatus !== "normal" && tempStatus !== prevState.temp) {
      const title =
        tempStatus === "warning" ? "⚠️ Peringatan Suhu" : "🚨 Bahaya Suhu";
